...
-- 創建數據庫（如果不存在）
CREATE DATABASE IF NOT EXISTS workt 
...
-- 創建workt用戶（如果不存在）
-- 注意：這需要root權限
CREATE USER IF NOT EXISTS 'workt'@'localhost' IDENTIFIED BY '4P7wf3n8inXCp4pZ';
CREATE USER IF NOT EXISTS 'workt'@'%' IDENTIFIED BY '4P7wf3n8inXCp4pZ';

-- 授予權限
GRANT ALL PRIVILEGES ON workt.* TO 'workt'@'localhost';
GRANT ALL PRIVILEGES ON workt.* TO 'workt'@'%';
FLUSH PRIVILEGES;
...
-- 創建索引以提高查詢性能（如果不存在）
CREATE INDEX IF NOT EXISTS idx_assignments_submit_time ON assignments(submit_time);
CREATE INDEX IF NOT EXISTS idx_assignments_group_name ON assignments(group_name);
CREATE INDEX IF NOT EXISTS idx_assignments_submitter_cookie ON assignments(submitter_cookie);

-- 顯示部署結果
SELECT 'Full deployment completed!' as Status;
SELECT 'Database created: workt' as Info;
SELECT 'User created: workt' as Info;
